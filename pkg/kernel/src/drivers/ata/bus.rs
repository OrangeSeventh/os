...
impl AtaBus {
    ...
    fn write_command(&mut self, drive: u8, block: u32, cmd: AtaCommand) -> storage::Result<()> {
        ...
        unsafe {
            ...
            // set driver selection and LBA mode
    ...
    pub(super) fn identify_drive(&mut self, drive: u8) -> storage::Result<AtaDeviceType> {
        ...
        if self.status().contains(AtaStatus::DRIVE_READY) {
            let mut data = [0u16; 256];
            for word in data.iter_mut() {
                *word = self.read_data();
            }
            Ok(match (data[0] & 0xFF00) >> 8 {
                0x00 => AtaDeviceType::Pata(Box::new(data)),
                0x14 => AtaDeviceType::PataPi,
                0x3C => AtaDeviceType::Sata,
                0x69 => AtaDeviceType::SataPi,
                _ => AtaDeviceType::None,
            })
        } else {
            // 设备未就绪，返回无设备
            Ok(AtaDeviceType::None)
        }
    }
