...
impl SemaphoreSet {
    pub fn insert(&mut self, key: u32, value: usize) -> bool {
        ...
        self.sems.insert(sid, Mutex::new(Semaphore::new(value))).is_none()
    ...
    pub fn wait(&self, key: u32, pid: ProcessId) -> SemaphoreResult {
        ...
        if let Some(semaphore) = self.sems.get(&sid) {
            let mut sem = semaphore.lock();
            sem.wait(pid)
        } else {
